...
function setupWorld() {
  ...
  var mat = new THREE.MeshPhongMaterial({color: 0x9dff3b5, overdraw: true});
  ...
  var material = new THREE.MeshNormalMaterial({overdraw: true, color: Math.random() * 0xFFFFFF, wireframe: true});
  ...
  for (var i = 0; i < 300; i++) {
    ...
    building.position.y = Math.floor( Math.random() * 200 - 50 ) * 4;
